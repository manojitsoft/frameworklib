...
public class BykerrPrefs {

    private static final String SHARED_PREF_NAME = "bykerr";
    private static Context mCtx;
    private static BykerrPrefs INSTANCE;
    private SharedPreferences prefs;

    private BykerrPrefs(Context context){
        this.mCtx = context;
        prefs = this.mCtx.getSharedPreferences(SHARED_PREF_NAME, Context.MODE_PRIVATE);
    }

    public static BykerrPrefs getInstance(Context context){
        if (INSTANCE == null)
            INSTANCE = new BykerrPrefs(context);
        return INSTANCE;
    }
